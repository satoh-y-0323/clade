...
フォアグラウンド実行（`background: false`）が明示されており、Claude Desktop などの環境でも必ずユーザーとの対話が可能な状態で起動します。ソースファイルの編集は行いません。

### /agent-architect

`requirements-report` を読み込み、システム設計・アーキテクチャ設計を行います。認証フロー・データモデル・API インターフェースなどを設計し、`architecture-report` として出力します。

フォアグラウンド実行（`background: false`）が明示されています。

### /agent-planner

`requirements-report` と `architecture-report` を読み込み、具体的な作業計画を立案します。タスクをマイルストーンに分割し、`plan-report` として出力します。

フォアグラウンド実行（`background: false`）が明示されています。ソースファイルの編集は行いません。

### /agent-developer

`plan-report` を読み込み、実装・デバッグを行います。
...
| `/agent-doc-writer` | Mermaid図・README・操作手順書・API仕様書などのドキュメント生成 |
...
Mermaid図・README・操作手順書・API仕様書・既存コードからの逆引き仕様書を生成します。生成前に対象ファイル・読み手・目的・粒度（高/中/低）をヒアリングするため、詳細すぎる出力を防げます。ソースコードの変更は行いません。
