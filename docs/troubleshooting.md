...
プロジェクトの `.claude/settings.local.json` が存在するか確認してください。このファイルがないとエージェントが正常に動きません。存在しない場合は、セットアップスクリプトを再実行するか、[はじめ方](/getting-started) の手順に従って手動作成してください。
...
セッション開始時に「逐次実行で続ける」を選択すると、確認ダイアログなしで作業を続けられます。
:::
