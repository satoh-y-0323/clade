...
`getting-started.md` にワークフロー選択ガイドを追加。直接指示・`/agent-developer`・フルワークフローの使い分けを具体例付きで解説。

### [v1.14.3] - 2026-04-14

`/init-session` にタスク規模トリアージを追加。新しいタスク開始時に small / medium / large を選ぶと、適切なワークフローを自動案内するようになった。

### [v1.14.2] - 2026-04-14

`setup.ps1` の文字化け修正。bash 経由で実行した際に日本語が CP932 で出力されていた問題を修正し、UTF-8 で正しく表示されるようにした。
