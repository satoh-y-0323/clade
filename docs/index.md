...
`/end-session` に昇格候補の提示機能を統合。セッション終了時にルール・スキル候補を一覧表示してその場で保存できるようになりました。docs に変更履歴ページを追加。

### [v1.9.0] - 2026-04-10

セッション開始時にセットアップ未実行を検出し、案内をユーザーに直接表示するよう改善。ドキュメントに `/update` コマンドの説明を追加。
