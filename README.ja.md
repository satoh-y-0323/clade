...
[![version](https://img.shields.io/badge/version-1.23.1-blue)](CHANGELOG.md)
...
VS Code 拡張には現在既知のバグがあり、プロジェクトレベルの `.claude/settings.json` に記載した `permissions.allow` が反映されません。その結果、フック実行のたびに確認ダイアログが表示されます。
...
- **逐次実行で続ける** — エージェントを1つずつ順番に実行します。すべての機能は正常に動作します。
