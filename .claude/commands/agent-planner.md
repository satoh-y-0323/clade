...
以下の4条件を**全て満たす**場合のみ、plan-report の**冒頭**（Markdown 本文より前）に
YAML フロントマターを出力する。満たさない場合はフロントマター自体を省略する。

**条件:**
1. 独立して実装できるタスクグループが 2つ以上存在する（互いに依存しない）
2. 実装ファイルが 3つ以上あり、かつ複雑なロジックを含む
3. 各グループが担当するファイルが明確に分離できる（ファイル重複禁止）
4. 共有インターフェース・型定義が事前に確定できる

> **注記:** clade-parallel の固定オーバーヘッドが10分以上あるため、単体実行が30分以上かかるものでないと並列化の効果がない。上記条件はこの基準を満たすタスクの目安として設定している。
...
| scale  | 担当タスク数の目安 | timeout | idle_timeout | 想定シナリオ |
|--------|-----|---------|-------------|-------------|
| small  | 3〜4 | 50分 | 40分 | 並列化の最低ライン。3〜4ファイル・単一機能の並列実装 |
| medium | 5〜8 | 100分 | 60分 | 標準的な並列実装（デフォルト）。複数機能にまたがる |
| large  | 9以上 | 200分 | 80分 | 複数サブシステムの同時実装・大規模リファクタ |

タスク数は目安であり、ファイル数が少なくても I/O が多い・ビルドが重い場合は large を選んでよい。
