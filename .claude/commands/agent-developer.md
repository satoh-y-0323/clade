...
各グループ（group-a, group-b, ...）に対して:
  Agent ツールで subagent_type: worktree-developer を run_in_background: true, isolation: "worktree" で起動する
  プロンプトに以下を含める:
    - グループID（例: group-a）
    - plan-report の当該グループのタスク一覧
    - 「最初のアクションとして .claude/group-config.json を Write で作成すること（内容: {"groupId": "group-a"}）」
```
