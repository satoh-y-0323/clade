...
CLADE_REPORT_EOF
   ...
   ```
   > **ヒアドキュメント構文の注意**: `CLADE_REPORT_EOF` は**行頭から書くこと（インデント禁止）**。
...
タスク一覧を組み立てた後、以下の条件を全て満たす場合は**並列開発モード**として plan-report に `parallel_groups` を定義する:

1. 独立して実装できるタスクグループが2つ以上存在する（互いに依存しない）
2. 各グループが担当するファイルが明確に分離できる
3. 共有インターフェース・型定義が事前に確定できる

並列開発モードの場合、plan-report の冒頭に **YAML フロントマター** として以下を出力する:
...
parallel_groups:
  group-a:
    ...
    tasks:
      - {タスクID}
    files:
      - {ファイルパターン（例: src/user/**, src/types/user.ts）}
  group-b:
    name: {グループ名}
    tasks:
      - {タスクID}
    files:
      - {ファイルパターン}
pre_implementation:
  - {並列開始前に実装・コミットが必要なファイル（インターフェース・型定義等）}
---
```

**ファイルパターンの記法:**
- `src/user/**` — `src/user/` 配下の全ファイル
- `src/types/user.ts` — 特定ファイル
- パターンはグループ間で重複しないよう定義すること

並列開発モードでない場合は YAML フロントマターを省略する。
