...
CLADE_REPORT_EOF
   ...
   **大規模レポート向け**: 追記モードで分割出力
   ...
   node .claude/hooks/write-report.js architecture-report new <<'CLADE_REPORT_EOF'
{冒頭のセクション（設計概要・アーキテクチャ図等）}
CLADE_REPORT_EOF
   # → 例: [write-report] .claude/reports/architecture-report-20260401-143022.md
...
CLADE_REPORT_EOF
   ...
   **⚠️ Bash が権限エラーで失敗した場合（最終手段）**: 単独で諦めることは禁止。
   レポートの全内容をインラインで出力し、「上記の内容を `.claude/reports/architecture-report-{タイムスタンプ}.md` に Write ツールで保存してください」と明記して終了する。
