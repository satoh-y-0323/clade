...
#### デベロッパーの責務（実装時）
`plan-report` を読み込んだとき、`milestone_mode` が記載されている場合は以下のルールに従う:

1. 現在のマイルストーンの全タスクが完了したら、そのマイルストーン単位でコミットを行う
2. コミット後の挙動はモードに依存する:
   - **`milestone_mode: confirm`** の場合:
     「マイルストーン〔N〕完了。コミットしました。続きのマイルストーン〔N+1〕を処理しますか？」をユーザーに表示して待機する。
     ユーザーが「はい」と答えた場合のみ次のマイルストーンへ進む。
     「いいえ」の場合はその時点で作業を終了し、再開方法をユーザーに案内する。
